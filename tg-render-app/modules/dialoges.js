...
const getAllDialogs = async (client, sortByName = true) => {
    try {
        ...
        fs.writeFileSync(path.join(EXPORT_DIR, "raw_dialog_list.json"), circularStringify(dialogs, null, 2));
        fs.writeFileSync(path.join(EXPORT_DIR, "dialog_list.html"), renderedHtml);
        fs.writeFileSync(path.join(EXPORT_DIR, "dialog_list.json"), JSON.stringify(dialogList, null, 2));
...
const getDialogName = async (client, channelId) => {
    try {
        const diaLogPath = path.join(EXPORT_DIR, "dialog_list.json");
        if (fs.existsSync(diaLogPath)) {
            ...
            if (dialog) return dialog.name;
        }

        // Fallback: resolve the name directly from Telegram instead of
        // requiring a prior "list dialogs" run first (needed for non-interactive/server use).
        const entity = await client.getEntity(channelId);
        return entity.title || entity.username || entity.firstName || String(channelId);
    } catch (error) {
