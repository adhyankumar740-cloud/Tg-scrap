...
class DownloadChannel {
  ...
  async downloadChannel(client, channelId, offsetMsgId = 0) {
    try {
      this.outputFolder = path.join(EXPORT_DIR, channelId.toString());
  ...
  async handle(options = {}) {
    // options.client - reuse an already-connected client (used by the web server
    //   so we don't reconnect for every channel)
    // options.exitProcess - defaults to true (original CLI behavior). Pass false
    //   when running inside a long-lived process like the Render web service.
